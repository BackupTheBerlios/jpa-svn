...
__version__ = '1.4.20040113b'    # History at the end of the file.
...
class Reader:
    ...
    def _readId3v1(self):
        ...
        self._addV1Frame('v1comment', tag[97:125])
    ...
    def dump(self):
        ...
        for fr in self.allFrames:
            if hasattr(fr, 'value'):
                print '%s: %s' % (fr.id, fr.value)
            else:
                print '%s= %s' % (fr.id, fr.rawData)
        for label in _simpleDataMapping.keys():
            v = self.getValue(label)
            if v:
                print 'Label %s: %s' % (label, v.encode('latin1', 'replace'))
